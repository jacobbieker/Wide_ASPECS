...
def get_observed_ghz(z, transition):
    """
    Get the observed GHz for given redshift based on transition
        ...
        for key, values in transitions.items():
            if values[0] < initial_catalog[id]['z'] < values[1]:
                ...
                if aspecs_lines[index]['rsnrrbin'] > 0.:
                    aspecs_redshifts.append((id, index, rest_ghz, diff_freq, key, aspecs_lines[index]['rsnrrbin'], delta_z))
        #print("\nMatch: " + str(index))
