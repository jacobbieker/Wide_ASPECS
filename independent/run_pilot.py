from match_sources import match_lines_to_catalog, make_skycoords
...
for option_name in ["all_closest"]:
    for separation in [1.2]:
        for snrlimit in [6.0,]:
            ...
            ascii.write(aspecs_table, "/home/jacob/Development/Wide_ASPECS/Pilot/ASPECS_Pilot_Line_Candidates_{}_Sep_{}_SN_{}.txt".format(option_name, separation, snrlimit), format='fixed_width', bookend=False, delimiter=None, overwrite=True, formats={'Observed CO (GHz)': '%3.4f', 'Restframe CO (GHz)': '%3.4f', 'Z (CO)': '%2.3f', 'Z (Matched)': '%2.3f',
            ...
            ascii.write(aspecs_table, "/home/jacob/Development/Wide_ASPECS/Pilot/ASPECS_Pilot_Line_Candidates_{}_Sep_{}_SN_{}.ecsv".format(option_name, separation, snrlimit), format='ecsv', overwrite=True)
            # Check if matched in the aspecs catalog

            combined_catalog = perform_cuts(combined_catalog)
            plot_mstar_vs_sfr(aspecs_catalog, combined_catalog, snr_limit=6.,z_lows=(0.0, 1.0, 2.0, 4), z_highs=(9.0, 2., 4, 9.), filename="/home/jacob/Development/Wide_ASPECS/Pilot/Pilot_Mstar_vs_SFR_{}_sep_{}_sn_{}.png".format(option_name, separation, snrlimit))