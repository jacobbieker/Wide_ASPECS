    ...
    for separation in [1.6]:
        for snrlimit in [0.0,]:
            ...
            ascii.write(aspecs_table,
                        ...
                        overwrite=True, formats={'RA (J2000)': '%2.6f', 'DEC (J2000)': '%2.6f', 'Roberto RA': '%2.6f',
            ...
            plt.errorbar(np.log10(aspecs_lines['mstar'])+9, np.log10(aspecs_lines['sfr']),
             yerr=(np.log10(aspecs_lines['sfr']) - (np.log10(aspecs_lines['sfr'] - aspecs_lines['sl_error'])), np.log10(aspecs_lines['sfr'] + aspecs_lines['su_error']) - np.log10(aspecs_lines['sfr'])),
             xerr=(np.log10(aspecs_lines['mstar'])+9 - (np.log10(aspecs_lines['mstar'] - aspecs_lines['ml_error'])+9),
                   (np.log10(aspecs_lines['mstar'] + aspecs_lines['mu_error'])+9) - (np.log10(aspecs_lines['mstar'])+9)), fmt='.',
             label='Pilot', c='g')
            '''
            ...
            for i, element in enumerate(aspecs_lines):
                plt.annotate(element['name'], (np.log10(aspecs_lines[i]['mstar'])+9, np.log10(aspecs_lines[i]['sfr'])))
            names = ["31","32","33","35"]
            for i, row in enumerate(aspecs_catalog):
                plt.annotate(names[i], (row['Mstar_50_1'], row['SFR_50_1']))
                print("Mstar: {} SFR: {} Z: {} Row: {}".format(row['Mstar_50_1'], row['SFR_50_1'], row['z_1'], i))
                print("Mstar2: {} SF2: {} Z2: {} Row {}\n".format(row['Mstar_50_2'], row['SFR_50_2'], row['z_2'], i))
            ...
            for index, line in enumerate(idx):
                ...
                print("Matched Line: RA: {} DEC: {} Row: {}\n".format(new_cat[idx[index]].ra.to_string(unit=u.hour, sep=':'), new_cat[idx[index]].dec.to_string(unit=u.degree, sep=':'), idx[index]))
            plt.legend(loc='best')
            #plt.show()
