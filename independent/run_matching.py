...
for index, one_in_special in enumerate(special_ones):
    combined_catalog = combine_catalogs(initial_catalog, roberto_catalog)
    new_ones = Table.read(one_in_special, format='fits')
    # Now get the max
    new_ones.sort(["Mstar_50", "SFR_50", "z_co", "z_1"])
    new_ones["id", "z_co", "z_1", "Mstar_50", "SFR_50", "Mstar_50_1", "SFR_50_1", "Mstar_50_2", "SFR_50_2"].pprint(max_lines=-1, max_width=-1)

    combined_catalog = perform_cuts(combined_catalog)
    plot_mstar_vs_sfr_third(new_ones, new_ones, combined_catalog, snr_limit=5., filename="Mstar_vs_SFR_{}_Add1.png".format(index))

combined_catalog = combine_catalogs(initial_catalog, roberto_catalog)
for option_name in ["all_closest", "all", "closest"]:
    for separation in [1.5]:
        for snrlimit in [6.0,5.9,5.8]:
