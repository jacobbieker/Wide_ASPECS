...
aspecs_table, aspecs_catalog = match_lines_to_catalog(aspecs_lines, combined_catalog, method='all', max_sep=1.0)

print(len(aspecs_table))
print("Len Matches: {}".format(len(aspecs_catalog)))
ascii.write(aspecs_table, "ASPECS_Line_Candidates_testOne.txt", format='fixed_width', bookend=False, delimiter=None, formats={'RA (J2000)': '%2.6f', 'DEC (J2000)': '%2.6f', 'Roberto RA': '%2.6f', 'Roberto DEC': '%2.6f','Observed CO (GHz)': '%3.4f', 'Restframe CO (GHz)': '%3.4f', 'Z (CO)': '%2.3f', 'Z (Matched)': '%2.3f',
